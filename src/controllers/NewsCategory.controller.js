// backend\src\controllers\NewsCategory.controller.js
import fs from "fs";
import path from "path";
import * as Category from "../models/NewsCategory.model.js";
import { slugify } from "../utils/slugify.js";
import { updateStatus } from "../models/updateStatus.model.js";

const BASE_URL = process.env.BASE_URL;
const IMAGE_DIR = "uploads/news-category";
...
//   const { rows, total } = await Category.getAllCategories(limit, offset);
...
export const activeCategories = async (req, res) => {
  const data = await Category.getAllCategoriesForNewsView();
  res.json({ success: true, data });
};
...
export const update = async (req, res) => {
  ...
  res.json({ success: true, message: "News category updated" });
};

export const toggleStatus = async (req, res) => {
  const { status } = req.body;
  const { id } = req.params;
  await updateStatus("tbl_newscategory", id, status);
...
export const viewCategoriesForClient = async (req, res) => {
  try {
    ...
    const makeImage = (img) =>
      img ? `${BASE_URL}/uploads/news-category/${img}` : "";
    ...
    res.status(500).json({
      success: false,
      message: "Failed to load News Category",
    });
  }
};
